...
> 작성: 2026-06-21 · 목적: 지금까지 개발된 전부를 한 곳에 정리하고, 다음 발전 아이디어를 brainstorming하기 위한 기준 문서.
...
- 폴드 커버(~280–360px) 단일컬럼+카드행 가로스크롤 / 펼침(~768–900px) 2컬럼, `safe-area`, `100dvh`.
- 설치: HTTPS(배포)에서 Chrome "홈 화면에 추가".

---

## 8. 온라인 멀티플레이 (Supabase)

- **DB**: 전용 `splendor` 스키마. `rooms`(code, config{players,seed}, actions[](액션소싱), seats{seat→client}, status, host) + `messages`(채팅). RLS 활성화(코드 기반 캐주얼 접근), Realtime publication. (`supabase/splendor_schema.sql`, `chat_schema.sql`)
- **클라이언트**(`lib/supabase.ts`, `store/gameStore.ts` 온라인 슬라이스):
  - 방 만들기/참여/좌석 점유/시작/나가기, **초대 링크 + 코드**
  - **동기화 = Realtime + 1.2초 폴링 폴백**(액션소싱이라 actions로 전체 상태 재구성)
  - 현재 좌석만 행동(`canActMain`/`controlsCurrent`), AI는 호스트 구동
  - **같은 인원 재게임**(시드만 새로, 좌석 유지) · **재접속 자동복귀**(방코드 localStorage) · **채팅**
- **검증**: `scripts/mp-test.mjs`(2-클라이언트 생성→참여→좌석→시작→행동 동기화 + 재게임), `reconnect-test.mjs`.
- **운영 주의**: 익명·방코드 접근 모델(서버측 규칙검증 없음), 방 row 미삭제(누적), NEXT_PUBLIC 키 필요(Vercel 환경변수 설정 완료).

---

## 9. 테스트 / 검증 자동화

- **vitest**: `lib/engine/*.test.ts`(엣지케이스 PRD 11장 + 2~4인 **fuzz/AI 시뮬레이션 수천 판**, 불변식: 토큰·카드 보존/한도/음수금지/종료·승자), `store/*.test.ts`(되돌리기·리플레이·수동지불·속도·예약구매·전원AI 완주).
- **Playwright 스크립트**(`scripts/`): `fold-test`(폴드/데스크 5뷰포트 오버플로0·에러0+스크린샷), `interact-test`(획득→되돌리기→구매→리플레이), `mp-test`/`reconnect-test`(온라인), `hover/drawer/howto-check`.
...
lib/ai/         휴리스틱 AI
lib/            supabase, assets(자동생성), effects(사운드/토스트), flyTrigger, sound
store/          gameStore(로컬+온라인+채팅), flyStore, toastStore + 테스트
components/     UI 전체(위 표)
public/         cards/ nobles/ gem/ icons/ + background.webp og.png manifest sw.js
supabase/       splendor_schema.sql, chat_schema.sql
scripts/        에셋/아이콘/OG 생성, 시뮬, Playwright 검증
...
- 온라인: 서버측 규칙 검증 없음(클라 신뢰), 방 row 미삭제(정리 필요), 폴링 1.2초 지연, 동시 append 희박한 경합 가능.
- 관전/재접속 중 AI 호스트 이탈 시 AI 정지 가능(호스트 위임 없음).
- 카드 일러스트는 레벨+색 공유(카드별 고유 아님), 일부 도트 일러스트는 카드 위 오버레이가 정보를 가릴 여지.
- 빌드는 exFAT에서 로컬 불가(Vercel 의존).
- i18n 없음(한국어 전용), 접근성(키보드/스크린리더) 미흡.
...
### A. 온라인/소셜 강화
- [ ] **관전 모드**(좌석 없이 보기) + 관전자 수 표시
- [ ] **호스트 위임/AI 자동 대리**: 호스트 이탈·연결 끊김 시 다른 클라가 AI 구동 인계, 비활성 플레이어 **턴 타임아웃→AI 대행**
- [ ] **재접속 안정화**: presence(온라인/오프라인 표시), 끊긴 좌석 표시, 방 자동 정리(만료 TTL)
- [ ] **서버측 규칙 검증**(Supabase Edge Function/RPC로 `append_action` 원자화 + 합법성 검사) → 부정행위 방지
- [ ] **빠른 매칭**(공개 방 목록/랜덤 매칭), 친구 초대, 관전 링크 분리
- [ ] **채팅 고도화**: 이모트/퀵챗, 시스템 메시지(행동 로그를 채팅에), 욕설 필터
...
### D. 메타/지속성
- [ ] **계정 + 통계**(Supabase Auth): 승률·평균 턴·귀족 획득률, 전적/리더보드
...
### F. 빠른 효과 큰 후보 (우선순위 제안)
1. **턴 타임아웃 + AI 대행** — 온라인 플레이 끊김 방지(체감 큼)
2. **관전 모드 + presence** — 친구와 보기 좋음
3. **서버측 액션 검증(Edge Function)** — 멀티 신뢰성/치팅 방지의 근본 해결
4. **통계/계정** — 재방문 동기
5. **튜토리얼** — 신규 유입 친화
```
